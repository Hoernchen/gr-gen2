...
class reader_monitor(gr.top_block):
    def __init__(self, u, trott, center_freq, hw_dec_rate, downsample_rate, pulse_width, tari, debug_on):
...
#            print "Couldn't set rx freq"
        ...
        sample_rate = 2e6 #self.u.adc_rate() / hw_dec_rate
        ...
        self.to_mag = blocks.complex_to_mag()
        ...
        self.smooth = blocks.moving_average_ff(int(pulse_width / us_per_sample) /2  , int(pulse_width / us_per_sample) /2 ) 
        
        self.rd = gen2.reader_decoder(us_per_sample, tari)

        self.fir_filter = fir_filter = firdes.low_pass(1, 2e6, 2e5, 2e4)
        self.freq_xlating_fir_filter_xxx_0 = filter.freq_xlating_fir_filter_ccc(1, (fir_filter), -4e5, 2e6)
        
        if(debug_on):
            self.sink = blocks.file_sink(gr.sizeof_float, "rm_thresh.out")
            self.signal_sink = blocks.file_sink(gr.sizeof_float, "rm_signal.out")
            self.connect(self.smooth, self.signal_sink)

        else:
            self.sink = gr.null_sink(gr.sizeof_float)
        
        self.connect(self.u, self.trott, self.freq_xlating_fir_filter_xxx_0, self.to_mag, self.smooth, self.rd, self.sink)
...
def main():
    ...
    downsample_rate = 1  #If you get 'S's, increase downsample_rate

    
    #tari = 7             # us. If you don't know, use 0. Then look at log to find actual TARI value.
    #pulse_width = 3.5      # us. 1/2 tari

    tari = 25             # us. If you don't know, use 0. Then look at log to find actual TARI value.
    pulse_width = 12.5      # us. 1/2 tari


    log_file = open("rm_log.log", 'w')

    blocks_file_source_0 = blocks.file_source(gr.sizeof_gr_complex*1, "/home/nxs/gqrx_20141203_120719_867700000_2000000_fc_snip1.raw", False)
    blocks_throttle_0 = blocks.throttle(gr.sizeof_gr_complex*1, 2e6, True)
        
   # u  = usrp2.source_32fc()
    
    
    rm = reader_monitor(blocks_file_source_0, blocks_throttle_0, center_freq, hw_dec_rate, downsample_rate, pulse_width, tari, DEBUG)
